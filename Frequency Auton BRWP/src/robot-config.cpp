  ...
  while(true) {
    if(RemoteControlCodeEnabled) {
     ...
     if (Controller1.ButtonB.pressing()){
      ...
      // only tell the left drive motor to spin if the values are not in the deadband range
      if (DrivetrainLNeedsToBeStopped_Controller1) {
        LeftDriveGroup.setVelocity(drivetrainLeftSideSpeed * 10, percent);
        LeftDriveGroup.spin(forward);
      }
      
      // only tell the right drive motor to spin if the values are not in the deadband range
      if (DrivetrainRNeedsToBeStopped_Controller1) {
        RightDriveGroup.setVelocity(drivetrainRightSideSpeed * 10, percent);
        RightDriveGroup.spin(forward);
      }
      ...
      if (Controller1.ButtonX.pressing()) {
        if (cautiousLift == 0){
          CataMotor.setVelocity(35, percent);
          cautiousLift = 1;
        } else {
      ...
      if (Controller1.ButtonA.pressing()){
        if(aClicked == 0){
        ogPos = CataMotor.position(degrees);
        wait(1, msec);
        CataMotor.setPosition(1, deg);
        aClicked = 1;
        } else{
          CataMotor.setPosition(ogPos, degrees);
          aClicked = 0;
        }
